...
@user_routes.route("/<int:id>/follow/<int:id2>", methods=["POST"])
@login_required
def follow(id, id2):
    current_user = User.query.get(id)
    second_user = User.query.get(id2)

    current_user.followers.append(second_user)
    db.session.commit()
    current_followers = User.query.filter(User.followers.any(id=id2)).all()
    return {"followers": [follower.to_dict() for follower in current_followers]}

# User wants to unfollow another user

@user_routes.route("/<int:id>/follow/<int:id2>", methods=["DELETE"])
@login_required
def unfollow(id, id2):
    current_user = User.query.get(id)
    second_user = User.query.get(id2)
